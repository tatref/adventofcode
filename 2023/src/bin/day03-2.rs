    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        for (line_number, line) in s.lines().enumerate() {
            let symbols: Vec<_> = line
                ...
                .collect();
            let symbols: Vec<(char, IVec2)> = symbols
                .iter()
                .filter_map(|(is_symbol, values)| {
                    if *is_symbol {
                        Some(
                            values
                                .iter()
                                .map(|(x, sym)| (*sym, IVec2::new(*x as i32, line_number as i32))),
                        )
                    } else {
                        None
                    }
...
fn compute(input: &str) -> u32 {
    ...
    let mut sum = 0;
    for &(_symbol, symbol_pos) in schematic.symbols.iter().filter(|(sym, _pos)| *sym == '*') {
        let mut gears = HashMap::new();
        for &number in &schematic.numbers {
            for offset in number.1.x..=number.2.x {
                let p = IVec2::new(offset, number.1.y);
                if (p - symbol_pos).length_squared() <= 2 {
                    gears.insert(number.1, number.0);
                }
            }
        }

        if gears.len() == 2 {
            sum += gears.values().product::<u32>();
        }
    }
